...
public final class IntentDetector {
    ...
    public static IntentType detect(String raw) {
        ...
        if (IntentUtils.includesAny(text, "증권", "증권으로", "송금", "송금하려", "이체", "옮기", "옮기려", "옮기려고", "돈 옮기", "돈 옮기려", "돈 옮기려고", "돈옮기", "보내", "보내려"))
        ...
        boolean mentionsBalance = IntentUtils.includesAny(text, "밸런스 2040", "kb밸런스", "안정성", "안정적", "안정");
        boolean mentionsDynamic = IntentUtils.includesAny(text, "다이나믹 주식형", "kb 다이나믹", "다이나믹");

        if (IntentUtils.includesAny(text, "두 펀드", "둘 중", "둘중", "펀드 차이", "둘 중에 뭐", "어느 쪽", "어떤 게 더", "비교해", "펀드차이")
                || mentionsBalance || mentionsDynamic) {
            if (IntentUtils.includesAny(text, "차이", "차이점", "비교")) return IntentType.DIFF_FUND;
        }

        if (IntentUtils.includesAny(text, "차이", "차이점", "무슨 차이", "차이는 뭐", "차이가 뭐", "어떤 차이", "비교", "비교해줘")) {
            return IntentType.DIFF_PRODUCT;
        }
        if ((mentionsPension && mentionsSip) ||
                (IntentUtils.includesAny(text, "상품 차이") && (mentionsPension || mentionsSip || IntentUtils.includesAny(text, "저축", "투자")))) {
            return IntentType.DIFF_PRODUCT;
        }
        ...
        if (mentionsBalance || mentionsDynamic)
            return IntentType.CHOOSE;

        if (IntentUtils.includesAny(text, "추세", "시장 추세", "요새 시장", "요즘 시장", "최근 시장", "시장 분위기", "시장 상황", "재분석", "다시 분석", "추세를 통해", "추세 기반", "시장", "요새"))
            return IntentType.TREND;
        if (IntentUtils.includesAny(text, "동향", "브리핑", "레포트", "간단하게", "간단한", "분석해서 설명", "분석해서", "분석해줘", "설명해줘", "설명해 줄래", "오늘"))
            return IntentType.BRIEFING;
        if (IntentUtils.includesAny(text, "추천", "추천할 만한", "추가 투자", "추가적인 투자", "포트폴리오 외", "다른 종목", "nvidia", "엔비디아"))
            return IntentType.RECOMMEND;
        if (IntentUtils.includesAny(text, "알림 설정", "알람 설정", "알림을 설정", "알림 맞춰", "가격 알림", "알림 등록", "알림 추가", "알림 켜", "nvidia 알림", "엔비디아 알림", "알림", "알람"))
